...
=item B<follow_task> is a boolean which dictates whether the selected line will stay selected after the task is chenaged.
...
=item B<silent_shell> is a boolean which defines whether task command output should be printed.
